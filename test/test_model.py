...
class testNet2:
    def __init__(self):
        ...
        self.sess.as_default()
        with tf.variable_scope('mytest'):
            self.x = tf.placeholder(shape=(None,2,3),dtype=tf.float32,name="input1")
            self.y = tf.placeholder(shape=(None,2,3),dtype=tf.float32,name="input2")
            # self.test_var = tf.Variable([1,2,3])
            self.ret1 = tf.add(self.x,self.y,"myadd")
            self.ret2 = tf.square(self.y,name="square")
